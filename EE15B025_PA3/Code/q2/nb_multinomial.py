...
with open("Result_nb_multinomial.txt", "w") as text_file:	
	f=0	
	for i in range(5):
		...
		for j,label in zip(range(len(labels)), labels):
			...
			for m in range(V):
				if m in textc[0]:
					Tct[m] = np.sum(X_train, axis=0)[m] #  Count of each word
			
			condprob[:,j] = ( Tct + np.ones_like(Tct) ) *1.0/ ( sum(Tct) + V ) # Conditional Probabilities
